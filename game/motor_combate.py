...
def processar_acao_jogador(jogador: dict, monstro: dict, skill_id: str) -> dict:
    ...
    if jogador['mana_atual'] < custo_mana:
        return {"log": f"Você não tem mana suficiente para usar {skill_data['nome']}!"}
    ...
    for efeito_id, valor in efeitos.items():
        if efeito_id == 'DANO':
            ...
            if efeito_id == "VIDA_MAXIMA":
                jogador['vida_atual'] += valor
            if efeito_id == "MANA_MAXIMA":
                jogador['mana_atual'] += valor
            log += f"\nSeu status de **{stat_alvo}** aumentou em `{valor}`!"

    monstro['vida_atual'] = max(0, monstro['vida_atual'] - dano_total_causado)
        
    # --- CORREÇÃO APLICADA AQUI ---
    # O retorno agora está limpo, sem a chave 'usos_habilidades_update'
    return {
